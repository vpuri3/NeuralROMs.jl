...
function advect2d_train_SNFL(
    ...
    λ1, λ2 = 0f0, 0f0     # L1 / L2 reg
    σ2inv, α = 1f-2, 1f-3 # code / Lipschitz regularization
...
# advect2d_train_DCAE(latent, modeldir_DCAE; device)
# # advect2d_train_CINR(latent, modeldir_CINR; device)
# advect2d_train_SNFW(latent, modeldir_SNFW; device)
# advect2d_train_SNFL(latent, modeldir_SNFL; device)
#
...
x0 = reshape(x0, 2, grid...)
# x1 = reshape(x1, 2, grid...)
x2 = reshape(x2, 2, grid...)
x3 = reshape(x3, 2, grid...)
#
x4 = reshape(x4, 2, grid...)
x5 = reshape(x5, 2, grid...)
x6 = reshape(x6, 2, grid...)

ud0, up0 = reshape.((ud0, up0), grid..., length(t0))
# ud1, up1 = reshape.((ud1, up1), grid..., length(t1))
ud2, up2 = reshape.((ud2, up2), grid..., length(t2))
ud3, up3 = reshape.((ud3, up3), grid..., length(t3))
#
ud4, up4 = reshape.((ud4, up4), grid..., length(t4))
ud5, up5 = reshape.((ud5, up5), grid..., length(t5))
ud6, up6 = reshape.((ud6, up6), grid..., length(t6))

#==================#
# save data
#==================#
filename = joinpath(@__DIR__, "advect2d.npz")
...
# error vs time
et0 = sum(abs2, e0; dims = (1,2)) / prod(size(e0)[1:2]) |> vec
# et1 = sum(abs2, e1; dims = (1,2)) / prod(size(e1)[1:2]) |> vec
et2 = sum(abs2, e2; dims = (1,2)) / prod(size(e2)[1:2]) |> vec
et3 = sum(abs2, e3; dims = (1,2)) / prod(size(e3)[1:2]) |> vec
#
et4 = sum(abs2, e4; dims = (1,2)) / prod(size(e4)[1:2]) |> vec
et5 = sum(abs2, e5; dims = (1,2)) / prod(size(e5)[1:2]) |> vec
et6 = sum(abs2, e6; dims = (1,2)) / prod(size(e6)[1:2]) |> vec
